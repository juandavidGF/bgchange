...
export default function ModelViewerPage({ slug, configurations }: { slug: string, configurations: Configuration[] }) {
  ...
  const loadGalleryItems = async (modelName: string) => {
    ...
    try {
      const response = await fetch(`/api/modelviewer/${modelName}`);
      if (response.ok) {
  ...
  const handleProcess = async () => {
    ...
    try {
      const response = await fetch(`/api/modelviewer/${selectedModel.name}`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <MinimalHeader
        title="Model Viewer"
        subtitle="Select models, configure parameters, and process your images"
      />
      
      <div className="h-screen bg-black text-white flex overflow-hidden">
        {/* Sidebar */}
        <div className="w-80 bg-black border-r border-gray-700 flex flex-col h-screen overflow-y-auto">
          {/* Model Selection */}
          ...
          {/* Parameters */}
          <div className="p-6 border-b border-gray-700 flex-1 overflow-y-auto">
